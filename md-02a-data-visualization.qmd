...
```{r}
ggplot(data = gapminder_2007, mapping = 
         aes(x = continent, y = lifeExp, fill = continent)) + 
         geom_boxplot(outlier.shape = NA)
# 3. I've used ?geom_boxplot to find out how to hide outliers
# 5. continent is a factor variable and lifeExp a double
```
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:  `geom_bar()` makes the height of the bar proportional to the number of cases in each group (or if the `weight` aesthetic is supplied, the sum of the weights).
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: the sequence specified in 'breaks' gives the bin boundaries.
